...
class AimsOutCalcChunk(AimsOutChunk):
    ...
    def _parse_eigenvalues(self):
        ...
        if self.n_k_points and (len(kpt_def) > 0):
            kpt_inds = [int(self.lines[ll].split()[1]) - 1 for ll in kpt_def]
        elif self.n_k_points:
            kpt_inds = [0]
        else:
            raise ParseError(
                f"The number of K-points is {self.n_k_points} but the number of defined K-points is {len(kpt_inds)}."
            )
