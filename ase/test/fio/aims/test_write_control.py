...
def test_control(create_au_bulk_atoms_obj):
    ...
    ase.io.aims.write_control(
        string_output, create_au_bulk_atoms_obj, parameters)
...
def test_control_wrong_tier(create_au_bulk_atoms_obj):
    ...
    with pytest.raises(ValueError, match='Given basis tier:'):
        ase.io.aims.write_control(
            io.StringIO(), create_au_bulk_atoms_obj, parameters)
...
def test_translate_tier(
    ...
    assert ase.io.aims.translate_tier(tier_as_str) == expected_tier_as_int
...
def test_format_tiers(
    ...
    (
        output_line,
        found_target_returned,
        do_uncomment_returned) = ase.io.aims.format_tiers(
...
def test_get_tier_list(tier, species, expected_tier_list):
    ...
    tier_list = ase.io.aims.get_tier_list(tier, species)
