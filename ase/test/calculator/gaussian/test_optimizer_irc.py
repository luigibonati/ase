...
def test_irc(atoms):
    ...
    calc_irc_for = get_calc(label='irc_for', oldchk=cwd + '/sp', chk='irc_for.chk')
    ...
    calc_irc_rev = get_calc(label='irc_rev', oldchk=cwd + '/sp', chk='irc_rev.chk')
...
#    assert e_ts - e_rev == pytest.approx(0.201, abs=1e-3)
    ...
    assert e_ts - e_for == pytest.approx(0.0, abs=1e-3)
    assert e_ts - e_rev == pytest.approx(0.0, abs=1e-3)
