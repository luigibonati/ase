...
class DftbProfile:
    ...
    def version(self):
        ...
        return check_output(self.argv + ['--version'])  # TODO: dftb+ does not have this option

    def run(self, directory, outputfile):
        from subprocess import check_call
        with open(outputfile, 'w') as fd:
            check_call(self.argv, stdout=fd,
                       cwd=directory)


class DftbTemplate(CalculatorTemplate):
    _label = 'dftb'  # Controls naming of files within calculation directory
...
class Dftb(GenericFileIOCalculator):
    ...
    def __init__(self, *, profile=None, directory='.', **kwargs):
        ...
        if profile is None:
            profile = DftbProfile(['dftb+'])

        super().__init__(template=DftbTemplate(),
                         profile=profile,
                         directory=directory,
                         parameters=kwargs)
