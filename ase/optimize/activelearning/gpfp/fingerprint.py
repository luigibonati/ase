...
class OganovFP():
    ...
    def calculate_gradient(self, index):
        '''
        Calculates the derivative of the fingerprint with
    ...
    def kernel_hessian(self, fp2, index1, index2):
        ...
        Qn = np.zeros(3)
        for B in range(self.n):
            Qm += (1 + int(B != A1)) * np.tensordot(tildexvec[B, A1],
                                                    g1[B], axes=[0, 0])

            Qn += - (1 + int(B != A2)) * np.tensordot(tildexvec[B, A2],
                                                      g2[B], axes=[0, 0])
        ...
        for B in range(self.n):  # sum over elements
            ...
            if A1 == A2:
                C += (1 + int(B != A2)) * np.tensordot(g1[B], g2[B],
                                                       axes=[0, 0])
    ...
    def dk_drm_dDelta(self, fp2, index):
        ...
        for B in range(self.n):
            ...
            for j in range(len(self.extendedatoms)):
                ...
                rij = self.rm[i, j]
                jsum += (1 + int(A == B)) * np.outer(self.dGij_dDelta(i, j),
                                                     -rij)
    ...
    def dGij_dDelta(self, i, j):
        ...
        first *= np.exp(-normsq / 2 / self.delta**2)
        second = 2 / xij * (normsq / self.delta**2 - 1)
        second += (xvec - xij) / self.delta**2 * (3 - normsq / self.delta**2)
