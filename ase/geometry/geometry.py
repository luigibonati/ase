...
def find_mic(v, cell, pbc=True):
    ...
    cell = complete_cell(cell)
    mr_cell, mr_path = minkowski_reduce(cell, pbc=pbc)
    if np.sign(np.linalg.det(cell)) != np.sign(np.linalg.det(mr_cell)):
        mr_cell = -mr_cell
    ...
    positions = wrap_positions(v, mr_cell, pbc=pbc, eps=0)
    vmin = positions.copy()
    vlen = np.linalg.norm(positions, axis=1)
    for nbr in neighbor_cells:
        trial = positions + np.dot(mr_cell.T, nbr)
